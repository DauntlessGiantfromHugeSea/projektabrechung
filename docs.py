...
Der Inhalt ist EINMAL strukturiert hinterlegt und wird daraus
- als Online-Hilfe (/anleitung) gerendert und
- als PDF (/anleitung.pdf, /anleitung-admin.pdf) erzeugt (fpdf2).

Blocktypen: p, h3, ul, ol, code, note, table, fig (nur online).
"""
...
def build_pdf(title: str, subtitle: str, section_groups: list[tuple[str, list[dict]]]) -> bytes:
    """PDF aus den Doku-Sektionen erzeugen. section_groups =
    [(Gruppentitel oder '', sections), ...]."""
    from fpdf import FPDF

    class _PDF(FPDF):
        def header(self):  # noqa: D401
            self.set_fill_color(146, 197, 122)
            self.rect(0, 0, self.w, 16, style="F")
            self.set_y(5)
            self.set_font(base_font, "B", 11)
            self.set_text_color(18, 48, 24)
            self.cell(120, 6, _tx(title), align="L")
            self.set_font(base_font, "", 9)
            self.cell(0, 6, _tx("Flüssigboden Engineering GmbH"), align="R",
                      new_x="LMARGIN", new_y="NEXT")
            self.set_y(22)

        def footer(self):
            self.set_y(-14)
            self.set_font(base_font, "", 8.5)
    ...
    unicode_ok = (_DEJAVU / "DejaVuSans.ttf").exists()
    if unicode_ok:
        pdf.add_font("dejavu", "", str(_DEJAVU / "DejaVuSans.ttf"))
        pdf.add_font("dejavu", "B", str(_DEJAVU / "DejaVuSans-Bold.ttf"))
        mono = str(_DEJAVU / "DejaVuSansMono.ttf")
        pdf.add_font("dejavumono", "", mono if Path(mono).exists()
                     else str(_DEJAVU / "DejaVuSans.ttf"))
        base_font, mono_font = "dejavu", "dejavumono"
    else:
        base_font, mono_font = "helvetica", "courier"

    def _tx(s: str) -> str:
        if unicode_ok:
            return s
        for a, b in _LATIN_MAP.items():
            s = s.replace(a, b)
        return s.encode("latin-1", "replace").decode("latin-1")

    def para(html_text: str, size=10.5, style="", color=(27, 42, 32),
             indent=0.0):
        pdf.set_font(base_font, style, size)
        pdf.set_text_color(*color)
        if indent:
            pdf.set_left_margin(16 + indent)
            pdf.set_x(16 + indent)
        pdf.multi_cell(0, 5.4, _tx(_strip_html(html_text)),
                       new_x="LMARGIN", new_y="NEXT")
        if indent:
    ...
    pdf.set_text_color(27, 42, 32)
    pdf.multi_cell(0, 10, _tx(title), new_x="LMARGIN", new_y="NEXT")
    pdf.set_font(base_font, "", 11)
    pdf.set_text_color(90, 105, 95)
    pdf.multi_cell(0, 6, _tx(subtitle), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)

    for group_title, sections in section_groups:
        if group_title:
            pdf.ln(2)
            pdf.set_fill_color(238, 245, 232)
            pdf.set_font(base_font, "B", 14)
            pdf.set_text_color(47, 107, 31)
            pdf.cell(0, 10, "  " + _tx(group_title), fill=True,
                     new_x="LMARGIN", new_y="NEXT")
            pdf.ln(2)
        for sec in sections:
            if pdf.get_y() > pdf.h - 45:
                pdf.add_page()
            pdf.set_font(base_font, "B", 12.5)
            pdf.set_text_color(47, 107, 31)
            pdf.cell(0, 8, _tx(sec["title"]), new_x="LMARGIN", new_y="NEXT")
            pdf.set_draw_color(146, 197, 122)
            ...
            for b in sec["blocks"]:
                ...
                if t == "p":
                    ...
                    pdf.set_text_color(27, 42, 32)
                    pdf.cell(0, 7, _tx(b["text"]), new_x="LMARGIN",
                             new_y="NEXT")
                elif t in ("ul", "ol"):
                    for i, item in enumerate(b["items"], 1):
                        ...
                        pdf.set_x(20)
                        pdf.multi_cell(
                            0, 5.4, _tx(f"{marker} {_strip_html(item)}"),
                            new_x="LMARGIN", new_y="NEXT")
                    pdf.ln(1)
                elif t == "code":
                    pdf.set_font(mono_font, "", 9)
                    pdf.set_text_color(40, 60, 45)
                    pdf.set_fill_color(244, 247, 241)
                    pdf.set_x(20)
                    pdf.multi_cell(0, 5.2, _tx(b["text"]), fill=True,
                                   new_x="LMARGIN", new_y="NEXT")
                    pdf.ln(1.5)
                elif t == "note":
                    pdf.set_font(base_font, "", 9.8)
                    pdf.set_text_color(110, 100, 60)
                    pdf.set_x(20)
                    pdf.multi_cell(0, 5.2,
                                   _tx("Hinweis: " + _strip_html(b["html"])),
                                   new_x="LMARGIN", new_y="NEXT")
                    pdf.ln(1.5)
                elif t == "table":
                    head, rows = b["head"], b["rows"]
                    w0 = 92.0
                    wn = (pdf.w - 32 - w0) / max(1, len(head) - 1)
                    pdf.set_font(base_font, "B", 9.5)
                    pdf.set_text_color(90, 105, 95)
                    pdf.set_fill_color(238, 245, 232)
                    for i, h in enumerate(head):
                        pdf.cell(w0 if i == 0 else wn, 7, _tx(h), fill=True,
                    ...
                    for row in rows:
                        for i, c in enumerate(row):
                            pdf.cell(w0 if i == 0 else wn, 6.6,
                                     _tx(_strip_html(c)),
                                     align="L" if i == 0 else "C", border="B")
                        pdf.ln(6.6)
                    pdf.ln(2)
                elif t == "fig":
                    pdf.set_font(base_font, "", 9.3)
                    pdf.set_text_color(130, 140, 130)
                    pdf.set_x(20)
                    pdf.multi_cell(0, 5, _tx(
                        f"[Abbildung: {b.get('caption', '')} – siehe "
                        "Online-Hilfe]"), new_x="LMARGIN", new_y="NEXT")
                    pdf.ln(1)
            pdf.ln(3)
